...
public:
	...
	//long int		RetrieveItem(long int index, VectorTypePtr inputItem);
...
//#############################################################################
//####################### Swap Item ####################################
/** Method for swapping a given index from the source vector to the target vector.
...
//#############################################################################
//####################### Retrieve Item ####################################
/** Method for retrieving a given index from the target vector that matches the source vector.
 * This method will only retrieve the index if the two vectors indexes match.

\param index Index to be searched for in the target
\param inputItem Target Vector

\version 1.0 2017/10/09
\author Gmo*/
//template<class VectorType>
//long int CHI_VECTOR<VectorType>::RetrieveItem(long int index, VectorTypePtr inputItem)
//{
//	int itemIndex = -1;
//	//===================================================== Check query within stack count bounds
//	if((index > this->stackCount) || (index < 0)){return NULL;};
...
//	{
//		if(inputItem[index] == inputItem[k])
//		{
//			itemIndex = k;
//			if(itemIndex == NULL)
//			{
//				printf("NULL found in: %p", &inputItem);
//				break;
//			}
//			break;
//		}
